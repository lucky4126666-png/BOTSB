...
def buttons(update: Update, context: CallbackContext):
    ...
    if data == "menu":
        ...
        query.edit_message_text("✏️ Gửi từ khoá cấm dưới tin nhắn này.", reply_markup=make_back_menu())
        ...
        query.edit_message_text("🚫 <b>BẤM vào từ muốn xoá:</b>", reply_markup=InlineKeyboardMarkup(btns), parse_mode=ParseMode.HTML)
...
def handle_text(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if context.user_data.get('awaiting_add_ban'):
        word = update.message.text.strip().lower()
        add_banned_word(word)
        update.message.reply_text(f"✅ Đã thêm từ cấm <b>{word}</b>!", parse_mode=ParseMode.HTML,
            reply_markup=full_menu(is_admin(user_id)))
        context.user_data['awaiting_add_ban'] = False
    else:
